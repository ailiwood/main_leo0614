#!/usr/bin/env python
"""P6H: TextFT-MinimalLoRA-AWAF MOSI training."""
import sys,os,json,time,torch,numpy as np,torch.nn as nn
...
from models.modules.minimal_lora import apply_lora_to_roberta, mark_only_lora_as_trainable, count_trainable
from utils.metrics import compute_all_metrics

DEVICE='cuda';BATCH=4;ACCUM=4;EPOCHS=50;LR=5e-5;SEED=42;H=256
...
# Text MLP
txt_mlp=nn.Sequential(nn.Linear(D,512),nn.LayerNorm(512),nn.GELU(),nn.Dropout(0.1),nn.Linear(512,H),nn.LayerNorm(H),nn.GELU(),nn.Dropout(0.1)).to(DEVICE)
reg_th=nn.Linear(H,1).to(DEVICE);cls_th=nn.Linear(H,1).to(DEVICE)

# Audio branch (frozen 768d features)
ap=nn.Sequential(nn.Linear(768,H),nn.LayerNorm(H),nn.GELU(),nn.Dropout(0.2)).to(DEVICE)
...
# Vision branch (frozen 768d features)
vp=nn.Sequential(nn.Linear(768,H),nn.LayerNorm(H),nn.GELU(),nn.Dropout(0.2)).to(DEVICE)
...
# Delta experts (conservative: max_delta=0.5)
def md(): return nn.Sequential(nn.Linear(H,H//2),nn.LayerNorm(H//2),nn.GELU(),nn.Dropout(0.2),nn.Linear(H//2,1)).to(DEVICE)
drt,dra,drv=md(),md(),md();dct,dca,dcv=md(),md(),md()
dsr=nn.Parameter(torch.tensor(0.02,device=DEVICE))
dsc=nn.Parameter(torch.tensor(0.02,device=DEVICE))
MAX_DELTA=0.5
...
opt=torch.optim.AdamW(params,lr=LR,weight_decay=0.03)
...
def forward_batch(batch):
    ...
    bdr=MAX_DELTA*torch.tanh(dr);bdc=MAX_DELTA*torch.tanh(dc)
    go=gate(ht,z,rtb,ctb,w,bdr);gr=go['gate_reg'];gc=go['gate_cls']
    edr=gr*dsr*bdr
    reg=rtb+edr;cls=ctb+gc*dsc*bdc
    return {'reg':reg,'cls':cls,'reg_text_base':rtb,'cls_text_base':ctb,'awaf_weights':w,'effective_delta_reg':edr,'delta_reg':dr,'residual_gate_reg':gr},lbl
...
for epoch in range(1,EPOCHS+1):
    ...
    for i,batch in enumerate(tqdm(tl,desc=f'E{epoch}',leave=False)):
        ...
        lr_loss=nn.L1Loss()(out['reg'],lbl)
        td=lbl-out['reg_text_base'].detach()
        ld=nn.SmoothL1Loss()(out['effective_delta_reg'],td)
        loss=(lr_loss+0.2*ld)/ACCUM
    ...
    if m['ACC2_Non0']>best_acc: best_acc=m['ACC2_Non0']

# Test
roberta.eval();txt_mlp.eval()
for m in [reg_th,cls_th,ap,aslstm,apool,vp,vslstm,vpool,awaf,gate,drt,dra,drv,dct,dca,dcv]: m.eval()
tp_list,tl_list,aw_list,gv_list,rb_list=[],[],[],[],[]
with torch.no_grad():
    for batch in tl_test:
        out,lbl=forward_batch(batch)
        tp_list.append(out['reg'].cpu());tl_list.append(lbl.cpu())
        aw_list.append(out['awaf_weights'].cpu());gv_list.append(out['residual_gate_reg'].cpu());rb_list.append(out['reg_text_base'].cpu())
rp=torch.cat(tp_list);tg=torch.cat(tl_list);rs=torch.where(rp>=0,1.0,-1.0)
aw=torch.cat(aw_list);gv=torch.cat(gv_list);rb=torch.cat(rb_list)
m=compute_all_metrics(rp,rs,tg);mb=compute_all_metrics(rb,rs,tg)
print(f'\n=== P6H MinimalLoRA-AWAF MOSI s{SEED} ({EPOCHS}ep) ===')
...
os.makedirs('outputs/P6H/mosi',exist_ok=True)
json.dump({'text_base_ACC2':mb['ACC2_Non0'],'final_ACC2':m['ACC2_Non0'],'MAE':m['MAE'],'Corr':m['Corr'],'ACC7':m['ACC7'],'F1':m['F1_Non0'],'awaf_w_t':float(aw[:,0].mean()),'awaf_w_a':float(aw[:,1].mean()),'awaf_w_v':float(aw[:,2].mean()),'gate_mean':float(gv.mean()),'trainable_M':n_trainable/1e6,'best_val_ACC2':best_acc},open('outputs/P6H/mosi/lora_seed42.json','w'))
print('Saved.')
